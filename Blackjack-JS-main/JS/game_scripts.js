...
function displayHands(playerHand, dealerHand, hideDealerCard = true) {
    let playerCards = playerHand.map(card => `${card.card} ${card.suit}`).join(", ");
    let dealerCards = hideDealerCard 
        ? `${dealerHand[0].card} ${dealerHand[0].suit}, ??` 
        : dealerHand.map(card => `${card.card} ${card.suit}`).join(", ");
    ...
    function handleAction(action) {
        ...
        if (action === 1) {
            ...
            if (dealer_total > 21) {
                ...
                message = "Dealer busted hands. You Win!";
